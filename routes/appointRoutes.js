...
let appointSchema = require('../models/appoint');
...
router.route('/getall/:role').get((req, res) => {
    ...
    if (req.params.role == "laboratory") {
        appointSchema.find({}).sort({ createdAt: -1 })
            .then(data => {
        ...
        appointSchema.find({ role: req.params.role }).sort({ createdAt: -1 })
            .then(data => {
                console.log("getall success");
                const decData = returnDecrypted(data);
                res.json(decData);
...
router.route('/findby').get((req, res) => {
    ...
    if (req.query.role == 'dentist') {
        if (req.query.name == "") {
            appointSchema.find({ role: req.query.role }).sort({ createdAt: -1 })
                .then(data => {
                    const decData = returnDecrypted(data);
                    console.log(decData);
                    res.json(decData);
            ...
            appointSchema.find({ name: encrypt(req.query.name), role: req.query.role }).sort({ createdAt: -1 })
                .then(data => {
                    const decData = returnDecrypted(data);
                    res.json(decData);
                })
        ...
        if (req.query.name == "") {
            ...
            appointSchema.find({ name: req.query.name })
                .then(data => {
                    const decData = returnDecrypted(data);
                    res.json(decData);
                })
...
router.route('/delete/:id').delete((req, res) => {
    appointSchema.findByIdAndDelete(req.params.id)
        .then((data) => {
            const decData = returnDecrypted(data);
            res.json(decData);
        })
